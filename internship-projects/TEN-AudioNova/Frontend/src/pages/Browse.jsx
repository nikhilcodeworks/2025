...
import React, { useState } from 'react';
import { IoPlay, IoPause, IoPlaySkipForward, IoPlaySkipBack, IoClose, IoVolumeHigh } from 'react-icons/io5';
  ...
  return (
    <div className="fixed bottom-8 right-8 w-96 bg-gradient-to-r from-gray-900/95 to-black/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/10 z-50 overflow-hidden">
      ...
      <div className="p-4">
        ...
        {/* Controls */}
        <div className="flex items-center justify-center space-x-4 mt-8 mb-3">
          <button onClick={onPrevious} className="text-white/70 hover:text-white transition-colors">
            <IoPlaySkipBack className="text-xl" />
          </button>
          <button
            onClick={onPlayPause}
            className="w-10 h-10 bg-gradient-to-r from-blue-500 to-purple-600 rounded-full flex items-center justify-center hover:from-blue-400 hover:to-purple-500 transition-all duration-200 shadow-lg"
          >
            {isPlaying ? <IoPause className="text-white text-lg" /> : <IoPlay className="text-white text-lg ml-0.5" />}
          </button>
